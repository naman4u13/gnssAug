...
public class EKF_PPP extends EKFParent {
	...
	public TreeMap<Long, double[]> process(TreeMap<Long, ArrayList<Satellite>> SatMap, ArrayList<Long> timeList,
			String[] obsvCodeList, boolean doAnalyze, 
			ArrayList<double[]> truePosEcefList, boolean isStatic, boolean repairCS) throws Exception {
		return process(SatMap, timeList, obsvCodeList, doAnalyze, truePosEcefList, isStatic,
				repairCS, false);
	}

	public TreeMap<Long, double[]> process(TreeMap<Long, ArrayList<Satellite>> SatMap, ArrayList<Long> timeList,
			String[] obsvCodeList, boolean doAnalyze,
			ArrayList<double[]> truePosEcefList, boolean isStatic, boolean repairCS, boolean fixAmb) throws Exception {
		...
		return iterate(SatMap, timeList, ssiSet, doAnalyze,truePosEcefList, obsvCodeList,
				repairCS, fixAmb, false);
	}

	TreeMap<Long, double[]> iterate(TreeMap<Long, ArrayList<Satellite>> SatMap, ArrayList<Long> timeList,
			ListOrderedSet ssiSet, boolean doAnalyze, 
			ArrayList<double[]> truePosEcefList, String[] obsvCodeList, boolean repairCS, boolean fixAmb,
			boolean doPseudorangeTest) throws Exception {
		...
		for (int i = 1; i < timeList.size(); i++) {
			...
			runFilter_vel(currentTime, deltaT, csdList, ssiSet, true, i, refPos,
					repairCS);
	...
	private void runFilter_vel(long currentTime, double deltaT, ArrayList<CycleSlipDetect> csdList,
			ListOrderedSet ssiSet, boolean useIGS, int ct,
...
//		}
	...
	private void runFilter_pos(long currentTime, double deltaT, ArrayList<CycleSlipDetect> csdList,
			...
			String[] obsvCodeList, boolean fixAmb,boolean doAnalyze, boolean doPseudorangeTest,double[] truePosRef) throws Exception {
...
//			System.out.println("Ionosphere Variance + " + mainVar.extractMatrix(coreStateNum + n, totalStateNum, 0, 1));
			...
			RotMat.insertIntoThis(0, 0, new SimpleMatrix(
					LatLonUtil.getEcef2EnuRotMat(truePosRef)));
	...
	private Object[] get_z_ze_H(SimpleMatrix x, int coreStateNum, int clkOffNum, int clkDriftNum, int ionoParamNum,
			...
			ArrayList<double[]> ionoParams, ArrayList<CycleSlipDetect> csdList,
