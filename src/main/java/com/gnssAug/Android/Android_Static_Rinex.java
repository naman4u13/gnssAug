...
import com.gnssAug.Rinex.estimation.EKF_PPP_LowCostRx;
...
public class Android_Static_Rinex {
	...
	public static void posEstimate(String obs_path,String osb_bias_path, String dcb_bias_path, String clock_path, String orbit_path,
		...
		try {
			...
			String path = "/Users/naman.agarwal/Library/CloudStorage/OneDrive-UniversityofCalgary/gnss_output/external_dataset/Pixel9XL_L1_GPS_PPP_noRepair";
			...
			HashMap<String, Object> ObsvMsgComp = ObservationRNX.rinex_obsv_process(obs_path, false, "",
			...
			// of frequency
			HashMap<String, double[]> rxPCO = new HashMap<String, double[]>();
			for(String code:obsvCodeList)
			{
				rxPCO.put(code,new double[] {0,0,0});
			}
			...
			for (ObservationMsg obsvMsg : ObsvMsgs) {
				...
				if (Math.abs(Time.getGPSTime(time)[0] -tRx)>0.9) {
					System.err.println("FATAL ERROR TIME calendar");
					return;
				}
				ArrayList<Satellite> satList = null;
				satList = com.gnssAug.IGS.SingleFreq.process(obsvMsg, NavMsgs, obsvCodeList, useIGS, useBias, ionoCoeff, dcb_bias,
...
//			}
			...
			for (String key : estVelMap.keySet()) {
				...
				for (int i = 0; i < n; i++) {
					...
					double[] enu = LatLonUtil.ecef2enu(velErr, trueEcef, false);
