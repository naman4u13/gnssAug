...
public class EstimatorPAR {
	...
	public static PARResult estimatorPAR(SimpleMatrix aHat, SimpleMatrix LMat, double[] dVec, Integer nCands,
		...
		if (alphaBIE > 0.0 && alphaBIE < 1.0) {
			...
			// NOTE: This uses the inverse of the regularized gamma function from Apache
			// Commons Math
			// Ensure that Apache Commons Math library is included in the project
			// dependencies
	...
	public static class PARResult {
		...
		public PARResult(SimpleMatrix aPAR, int nFixed, double SR_PAR, Object[] stats) {
			this.aPAR = aPAR;
			this.nFixed = nFixed;
			this.SR_PAR = SR_PAR;
			this.stats = stats;
