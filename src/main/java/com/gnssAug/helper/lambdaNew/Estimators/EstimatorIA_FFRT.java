...
public class EstimatorIA_FFRT {
	 ...
	 */
	public class IAFFRTResult {
		private SimpleMatrix aFix;
		private double sqNorm;
		private int nFixed;

		private Double muRatio;

		public IAFFRTResult(SimpleMatrix aFix, double sqNorm, int nFixed, Double muRatio) {
			this.aFix = aFix;
			this.sqNorm = sqNorm;
			this.nFixed = nFixed;
	 ...
	 */
	public IAFFRTResult estimatorIA_FFRT(SimpleMatrix aHat, SimpleMatrix lMat, double[] dVec, Double maxFR,
			Double muRatio) {
		...
		double effectiveMaxFR = (maxFR != null) ? maxFR : 1 / 100;
		...
		if (fr < effectiveMaxFR) {
			...
			return new IAFFRTResult(aFix, sqNorm, nFixed, 1.0);
		} else {
			...
			if (sqNormTemp[0] / sqNormTemp[1] > muValue) {
				...
				return new IAFFRTResult(aFix, sqNorm, nFixed, muValue);
			} else {
				SimpleMatrix aFix = aFixTemp.extractVector(false, 0);
				double sqNorm = sqNormTemp[0];
				int nFixed = nn;
