...
public class SINEX {
	...
	private static Map<String, Object> compute_ARP_PCO(double[] MM, double[] deltaARP,
			HashMap<Character, HashMap<Integer, double[]>> PCO, String[] obsvCodeList) {
		int fN = obsvCodeList.length;
		HashMap<String, double[]> PCO_ECEF = new HashMap<String, double[]>();

		double[] ARP = LatLonUtil.enu2ecef(deltaARP, MM, true);
		for (int i = 0; i < fN; i++) {
			...
			double[] APC = LatLonUtil.enu2ecef(pco, ARP, true);
			for (int j = 0; j < 3; j++) {
				pco_ecef[j] = APC[j] - ARP[j];
			}
			PCO_ECEF.put(obsvCode, pco_ecef);
