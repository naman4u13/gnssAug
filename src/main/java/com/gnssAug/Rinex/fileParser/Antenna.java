...
public class Antenna {
	...
	public double[] getSatPC_windup(int SVID, String obsvCode, double GPSTime, long weekNo, double[] satMC) {
		...
		return getSatPC_windup(SVID, new String[] { obsvCode }, GPSTime, weekNo, satMC)[0];
	...
	public double[][] getSatPC_windup(int SVID, String[] obsvCode, double GPSTime, long weekNo, double[] satMC) {
		...
		/*
		 * double[] userLL = LatLonUtil.ecef2lla(userECEF); double[] unitLOS =
		 * SatUtil.getUnitLOS(Arrays.copyOfRange(satPC_windUp[0], 0, 3), userECEF);
		 * double[] ar = new double[] { -Math.sin(userLL[1]), Math.cos(userLL[1]), 0 };
		 * double[] br = new double[] { -Math.sin(userLL[0]) * Math.cos(userLL[1]),
		 * -Math.sin(userLL[0]) * Math.sin(userLL[1]), Math.cos(userLL[0]) }; double[]
		 * as = i; double[] bs = j;
		 * 
		 * double[] k_cross_br = Vector.crossProd(unitLOS, br); double[] k_cross_bs =
		 * Vector.crossProd(unitLOS, bs); double k_dot_ar = Vector.dotProd(unitLOS, ar);
		 * double k_dot_as = Vector.dotProd(unitLOS, as); double[] Dr =
		 * Vector.add(Vector.subtract(ar, Vector.scale(unitLOS, k_dot_ar)), k_cross_br);
		 * double[] Ds = Vector.subtract(Vector.subtract(as, Vector.scale(unitLOS,
		 * k_dot_as)), k_cross_bs); double sign = Math.signum(Vector.dotProd(unitLOS,
		 * Vector.crossProd(Ds, Dr))); double windUpCycle = (sign *
		 * (Math.acos(Vector.dotProd(Ds, Dr) / (Vector.mod(Ds) * Vector.mod(Dr))))) / (2
		 * * Math.PI);
		 * 
		 * IntStream.range(0, fN).forEach(index -> satPC_windUp[index][3] =
		 * windUpCycle);
		 */
